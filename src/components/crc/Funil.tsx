...
export function Funil({ aoAbrirPaciente }: { aoAbrirPaciente: (patientId: string) => void }) {
  ...
  const filtrosAtivos =
    filtro.tipos.length + (filtro.etapaChave === null ? 0 : 1) + (filtro.apenasMinhas ? 1 : 0);
    ...
    <div className="crc-funil-v2">
      ...
      <section className="crc-funil-resumo-v2" aria-label="Resumo do pipeline">
        <ResumoFunil
          ...
          nota="No recorte atual"
        />
        <ResumoFunil
          icone={Flame}
          rotulo="Prioridade alta"
          valor={altas}
          nota={altas > 0 ? "Pedem atenção primeiro" : "Nenhuma crítica agora"}
          tom={altas > 0 ? "perigo" : "positivo"}
        />
        <ResumoFunil
          icone={Bot}
          rotulo="Com automação"
          valor={emAutomacao}
          nota="Jornada ativa conduzindo"
          tom="info"
        />
        <ResumoFunil
          icone={Filter}
          rotulo="Filtros ativos"
          valor={filtrosAtivos}
          nota={filtrosAtivos > 0 ? "Quadro recortado" : "Mostrando tudo"}
        />
      </section>
...
function CartaoFunil({
  ...
  return (
    <article
      className={`crc-cartao-funil crc-funil-card-v2 crc-prioridade crc-prioridade-${cartao.faixa}`}
      data-faixa={cartao.faixa}
    >
      <div className="crc-funil-card-topo-v2">
        ...
        <Etiqueta
          tom={cartao.faixa === "ALTA" ? "perigo" : cartao.faixa === "MEDIA" ? "alerta" : "neutra"}
        >
          {cartao.score}
        </Etiqueta>
      </div>
      ...
      <div className="crc-funil-card-dados-v2">
        {cartao.valorPotencial !== null && (
          <span>
            ...
            <strong>{dinheiro(cartao.valorPotencial)}</strong>
          </span>
        )}
        {cartao.proximaAcao !== null && (
          <span>
            <small>Próxima ação</small>
            <strong>{cartao.proximaAcao}</strong>
          </span>
        )}
      </div>

      {cartao.temJornadaAtiva && (
        <div className="crc-funil-automacao-v2">
          <Sparkles aria-hidden="true" /> Automação conduzindo este caso
        </div>
      )}
      ...
      <div className="crc-funil-card-acoes-v2">
        ...
        {cartao.patientId !== null && (
          <Botao
            pequeno
            variante="discreto"
            onClick={() => cartao.patientId !== null && aoAbrirPaciente(cartao.patientId)}
          >
