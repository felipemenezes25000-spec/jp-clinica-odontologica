...
/**
 * LANDING PAGE DE ANÚNCIO — "Dentista infantil".
 *
 * Mesma página de `/tratamentos/odontopediatria`, sob a URL que casa com o
 ...
 * de "dentista infantil" precisa ver, na barra de endereço e no título, a
 ...
 */
const SLUG = "odontopediatria";
const CANONICO = `${SITE_URL}/tratamentos/odontopediatria`;
...
export const Route = createFileRoute("/odontopediatria")({
  ...
  head: () => {
    const t = TRATAMENTOS.find((x) => x.slug === SLUG);
    if (!t) return {};
    const title = tituloLocal("Dentista infantil");
    const description = descricaoLocal(t.desc, FECHO_LOCAL);
    const url = `${SITE_URL}/odontopediatria`;
    return {
      meta: [
        { title },
