...
/**
 * LANDING PAGE DE ANÚNCIO — "Aparelho e ortodontia".
 *
 * Mesma página de `/tratamentos/ortodontia`, sob a URL que casa com o
 ...
 * de "aparelho e ortodontia" precisa ver, na barra de endereço e no título, a
 ...
 */
const SLUG = "ortodontia";
const CANONICO = `${SITE_URL}/tratamentos/ortodontia`;
...
export const Route = createFileRoute("/ortodontia")({
  ...
  head: () => {
    const t = TRATAMENTOS.find((x) => x.slug === SLUG);
    if (!t) return {};
    const title = tituloLocal("Aparelho e ortodontia");
    const description = descricaoLocal(t.desc, FECHO_LOCAL);
    const url = `${SITE_URL}/ortodontia`;
    return {
      meta: [
        { title },
