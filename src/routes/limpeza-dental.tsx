...
/**
 * LANDING PAGE DE ANÚNCIO — "Limpeza dental".
 *
 * Mesma página de `/tratamentos/limpeza-profilaxia`, sob a URL que casa com o
 ...
 * de "limpeza dental" precisa ver, na barra de endereço e no título, a
...
export const Route = createFileRoute("/limpeza-dental")({
  ...
  head: () => {
    const t = TRATAMENTOS.find((x) => x.slug === SLUG);
    if (!t) return {};
    const title = tituloLocal("Limpeza dental");
    const description = descricaoLocal(t.desc, FECHO_LOCAL);
    const url = `${SITE_URL}/limpeza-dental`;
    return {
      meta: [
        { title },
