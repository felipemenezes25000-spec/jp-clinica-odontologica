...
_O schema._ Os arquivos 16 a 19 foram aplicados em produção e verificados um a
...
_As fases G a I._ Foram feitas, e a ponta visível delas também — quatro telas,
nove funções de servidor e o endpoint MCP. Ver "A ponta visível", no topo.

_O caso de tenant no gate._ O item 31 é `PASS`: as quatro categorias
...
_Os três P0 de recuperação da auditoria externa._ Foram corrigidos nesta rodada
...
para quatro. Em ambos, quem chamava era _obrigado_ a tratar o caso grave como se
