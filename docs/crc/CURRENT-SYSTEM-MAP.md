...
- **Server functions** (`createServerFn`) em um arquivo importável pelo cliente;
  todo módulo de servidor entra por `await import()` *dentro* do handler.
...
- Nomes de domínio em português; comentário explica *por quê*, não *o quê*.
...
1. **Fatiamento do bundle SSR** — `vite.config.ts` força
   ...
   que derrubou *todas* as rotas com o build passando limpo
